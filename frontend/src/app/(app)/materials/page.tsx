...
function PurchaseFormDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  const { t } = useTranslation();
  const qc = useQueryClient();
  ...
  React.useEffect(() => {
    setPurity(PURITY_OPTIONS[material][0]);
  }, [material]);
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{t("material.newPurchase")}</DialogTitle>
          <DialogDescription>Record a new raw material purchase entry</DialogDescription>
        </DialogHeader>
