...
export default function DashboardPage() {
  ...
  const trends = useQuery({
    queryKey: ["trends"],
    queryFn: () => api("/api/dashboard/trends"),
  });
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          title={t("dashboard.totalGold")}
          value={`${formatNumber(summary.data?.goldStock ?? 0)} g`}
          subtitle="All purities combined"
          icon={Coins}
          accent="gold"
        />
        <StatCard
          title={t("dashboard.totalSilver")}
          value={`${formatNumber(summary.data?.silverStock ?? 0)} g`}
          subtitle="925 + 999 combined"
          icon={Gem}
          accent="silver"
        />
        <StatCard
